...
# Email Configuration (recommended: set via environment variables)
...
# Comma-separated list of admin notification emails (used when feedback is submitted)
...
class Feedback(db.Model):
    ...
    category    = db.Column(db.String(50), nullable=False)  # fraud, technical, general, etc.
    message     = db.Column(db.Text, nullable=False)
...
def send_email(to_email, subject, body_html):
    ...
    try:
        ...
        err = str(e)
        print(f"Email send error to {to_email}: {err}")
...
def run_prediction(data):
    ...
    try:
        ...
        # Gracefully handle empty or invalid inputs
        return {
    ...
    # Validate transaction date against dataset range and current date.
    try:
